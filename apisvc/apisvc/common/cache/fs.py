...
def get_account(account):
    ...
    if os.path.isdir(account_cached):
        LOGGER.debug('account {0} found in the cache'.format(account))
        return account_cached
    else:
        LOGGER.debug('account {0} not found in the cache'.format(account))
        return None
...
def put_credential(account, credential_k8s, credential_os):
    ...
    if account_cached is None:
        ...
        LOGGER.debug('account {0} saved in the cache'.format(account))
    ...
    with open(credential_k8s_cached, 'w') as file_k8s:
        ...
        LOGGER.debug('credential {0} saved in the cache'.format(credential_k8s_cached))

    with open(credential_os_cached, 'w') as file_os:
        file_os.write(credential_os)
        LOGGER.debug('credential {0} saved in the cache'.format(credential_os_cached))
